...
public class InterfazMail {
    public static void notificarNovedadesImportadas(List<String> listaMails, Sismografo sismografoSeleccionado) {
        Timer timer = new Timer(3000, e -> {
            JFrame frame = new JFrame("Ventana Mail");
            ...
            // Lista de mails
            JList<String> mailsList = new JList<>(listaMails.toArray(new String[0]));
            JScrollPane scrollPane = new JScrollPane(mailsList);
            panel.add(scrollPane, BorderLayout.CENTER);

            // Panel inferior
            JPanel southPanel = new JPanel(new BorderLayout());

            JLabel leyenda = new JLabel("La notificación fue enviada con éxito.", JLabel.CENTER);
            leyenda.setFont(new Font("Arial", Font.ITALIC, 24));
            southPanel.add(leyenda, BorderLayout.NORTH);

            // Crear datos para la tabla con reflexión
            ...
            tabla.setEnabled(false); // Que no se pueda editar
            tabla.setFont(new Font("Arial", Font.PLAIN, 12));
            tabla.setRowHeight(20);

            southPanel.add(tablaScroll, BorderLayout.CENTER);
